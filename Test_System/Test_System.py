...
if app.is_tsmaster_host(): # only vaid in TSMaster App
    app.import_mini_program_header("ts_lib")
    globals()["ts_lib"] = sys.modules.get("ts_lib")
            ...
            for case_id, lib, func_name in checked:
                ...
                try:
                    ...
                    if mod is None:
                        ...
                        continue
                    print(f"get {func_name}")
                    func = getattr(mod, func_name)
                    func()
                    self._result_queue.put((case_id, 'Passed'))
                except (ModuleNotFoundError):
                    print(f"ModuleNotFoundError,{lib}")
                    self._result_queue.put((case_id, 'No Case'))
                except (AttributeError):
                    print(f"AttributeError,{lib}.{func_name}")
                    self._result_queue.put((case_id, 'No Case'))
                except Exception as e:
                    print(e)
